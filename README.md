...
- **The wire contract is authored here, and so are both sides of it.** [`docs/server-mode.md`](docs/server-mode.md) is the spec of record — single HTTP port, WebSocket relays for providers and KYOK bridges, the ticket handshake. The SDKs that implement that spec live in this repo too ([`souk-agent-sdk/`](souk-agent-sdk) for providers, [`souk-client-sdk/`](souk-client-sdk) for callers and their KYOK bridges): upstream keeps no network code at all, client side included. The signed payloads inside the frames are upstream's, pinned at **contract revision 23** and vendored as [`docs/upstream-contract-vectors.json`](docs/upstream-contract-vectors.json) — and since revision 23 that file publishes *only* what a signature covers, so the two delivery envelopes are vectored here instead, in [`docs/wire-vectors.json`](docs/wire-vectors.json), beside every other frame on this wire ([funduq#282](https://github.com/hukaichun/funduq/issues/282), filed from here). Upstream withdrew its own frame vocabulary and protocol machines at revision 11 — over a wire the provider-initiated calls are plain request/response — so the framing on this wire is stated and tested here; what it publishes instead is every crossing shape as a pydantic model. The one ordering that turned out to live inside core rather than on the wire went back there at revision 17 ([funduq#249](https://github.com/hukaichun/funduq/issues/249)).
- **funduq core arrives from PyPI** (`funduq` 0.0.11, `funduq-provider-sdk[llm]` 0.0.9, `funduq-contract` 0.0.13 — version-pinned in `pyproject.toml`). This repo contains no domain logic — it lifts headers, frames responses, binds sockets, and hands everything else to core.
- **The gateway authenticates a caller that presents a chain, and only that caller.** Contract revision 21 refuses an actor chain handed to a door that cannot say who presented it, so this gateway grew the authenticating seat upstream has always assumed in front of it: a `Funduq-Presenter` header, compact JSON `{publicKey, timestamp, signature}`, signed over `funduq-server-presenter:{public_key}:{timestamp}:{sha256hex(body)}` and fresh for 60 seconds. **A caller without a chain is unaffected and still anonymous** — no header, no change. This is not "the gateway grew auth"; it is "a party that already holds a key can prove it". The one invented header this repo now has (`X-Funduq-View` is deleted); designed in [`docs/server-mode.md`](docs/server-mode.md#the-authenticating-seat), vectored in [`docs/wire-vectors.json`](docs/wire-vectors.json).
- **Where every authorization decision can live is inventoried once**, in [`docs/authorization.md`](docs/authorization.md): three layers — the record's own facts (core's, not delegable), who is at the door (this gateway's, necessarily), and whether an identity is entitled here (the deployment's, and this repo has no way to express it yet). It is the document to read before adding a door or a check to one.
- **Who may read the record is this repo's rule, and it is written once.** Contract revision 22 took the question out of core ([funduq#275](https://github.com/hukaichun/funduq/issues/275)): core derives *who the parties are* (`Funduq.parties_of` — the head, the serving provider, every key on the thread's runs' chains), because the chains are verified there and nowhere else, and the rule reading that answer is [`souk_server/reads.py`](souk_server/reads.py). The circle is exactly the one revision 21 enforced — nothing about who sees what changed here — and its callers are the three A2A read operations and the provider socket's thread query. A deployment that wants a different rule has one function to change instead of four doors to find.
...
> **Heads up on an existing database: this migration deletes history.** The current chain ends at alembic revision `a1f4c9d27e3b`, which reshaped `runs` around AG-UI's `RunAgentInput` (contract revision 19), and upstream chose not to migrate rows — the upgrade **deletes every row in `runs`, `run_events` and `thread_messages`**. Agents, providers and threads survive; conversations do not. `souk-migrate` runs it for you on `docker compose up`, so back up first if that data matters.
