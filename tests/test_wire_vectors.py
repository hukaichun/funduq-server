...
does not define — each socket's frame vocabulary, and, since contract
revision 23, the two delivery envelopes. The other consumers (souk-agent-sdk,
...
def test_the_published_envelopes_are_what_the_models_dump():
    """The envelopes arrived here at contract revision 23, and an owner
    owes more than keeping them present.

    Upstream deleted its `wire` section because a vector belongs in that
    file only when getting it wrong fails a signature check, and nothing
    signs an envelope (funduq#282) — then added a test so the section
    cannot quietly return. The mirrored obligation is this one: the side
    that now publishes these bytes is the side that has to keep them
    *true*, and only this module can, because only this module imports the
    models they are a dump of.

    So the assertion is a round trip rather than a field list. The three
    consumers — both SDK suites and the Go probe — can only check that
    the frames are there and that a few fields read the way they expect;
    none of them would notice the day a crossing shape gains, loses or
    renames a field and the published frame stops being what
    `model_dump(by_alias=True)` produces. That drift is exactly what a
    vector exists to catch, and it would otherwise be caught by a
    provider in production instead.

    **`runInput` is held out of the value comparison, not out of the
    test.** Its field names still have to match, because that is where a
    crossing shape would drift; its contents are `ag_ui.core`'s model, and
    the frame was generated by an ag-ui that emits `parentRunId: null` and
    `resume: null` where 0.1.22 — this repo's floor — omits both when
    unset. Both are optional with a `None` default on either side, so the
    skew is invisible to a run, and asserting it would pin an ag-ui
    version rather than the envelope this test is about. The same holdout,
    for the same reason, is written out in
    `souk-agent-sdk/tests/test_shapes_and_proofs.py`.
    """
    envelopes = VECTORS["envelopes"]

    assert {"delivered_run", "delivered_completion"} <= set(envelopes)

    for name, model, third_party in (
        ("delivered_run", funduq_contract.DeliveredRun, "runInput"),
        ("delivered_completion", funduq_contract.DeliveredCompletion, None),
    ):
        frame = envelopes[name]["frame"]
        # by_alias=True and never exclude_none — the frame envelope half of
        # the two dump rules in docs/server-mode.md. `exclude_none` here
        # would strip `forwardedProps`, which this very frame carries as a
        # populated dict precisely so the rule has something to protect.
        dumped = model.model_validate(frame).model_dump(by_alias=True, mode="json")

        assert set(dumped) == set(frame), name
        assert {k: v for k, v in dumped.items() if k != third_party} == {
            k: v for k, v in frame.items() if k != third_party
        }, name
